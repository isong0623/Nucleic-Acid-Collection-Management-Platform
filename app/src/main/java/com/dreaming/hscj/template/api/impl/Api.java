...
public class Api {
    ...
    public static int typeOf(String text){
        try {
    ...
    public static String fieldOf(String text){
        try {
    ...
    protected long getInterval(){
        return Constants.Net.isDebug ? 0 : interval;
    }
...
//            loggingInterceptor.setPrintLevel(HttpLoggingInterceptor.Level.BODY);Log.打印级别，决定了log显示的详细程度
//            loggingInterceptor.setColorLevel(Level.INFO);                       Log.颜色级别，决定了log在控制台显示的颜色
...
//        builder.addInterceptor(new Interceptor() { //全局拦截器
//            @Override
...
//                        .addHeader("Authorization",  Constants.User.getToken())
//                        .method(originalRequest.method(),originalRequest.body());
//
//                return chain.proceed(requestBuilder.build());
//            }
//        });
        OkGo.getInstance().init(App.sInstance)                  //必须调用初始化
                .setOkHttpClient(builder.build())                   //建议设置OkHttpClient，不设置将使用默认的
                .setCacheMode(CacheMode.FIRST_CACHE_THEN_REQUEST)   //全局统一缓存模式，默认不使用缓存，可以不传
                .setCacheTime(CacheEntity.CACHE_NEVER_EXPIRE)       //全局统一缓存时间，默认永不过期，可以不传
                .setRetryCount(1);                                  //全局统一超时重连次数为1次，那么最差的情况会请求2次(一次原始请求，一次重连请求)，不需要可以设置为0
    }

    private AtomicLong atmLastRequestTimestamp = new AtomicLong(0);
    public void doRequestInternal(List<Object> requestParams, ApiListener listener, StringBuilder sb){
        ...
        try{
            if(listener!=null) post(()->{ try{ listener.onPerform(); }catch (Exception e){e.printStackTrace();} });
            ...
            for(int i=0,z=4;i<lstHeaders.size();++i){
                ...
                if(permission.getBAllowRememberPassword() &&
                    ...
                    for(int j=0;j<3 && !isLoginSuccess.get();++j){
                        ...
                        Template.getCurrentTemplate().apiOf(0).doRequestInternal(lst, new ApiListener() {
                            @Override
                ...
                App.Post(()-> ToastUtils.show("当前未登录或登录失效，请重新登录！"));
                sb.append("\n");
                sb.append("请求中断！\n");
            ...
            if("JSON".equalsIgnoreCase(getRequest().getUpload())){
                ...
                switch (getRequest().getType()){
                    ...
                    case "POST":
                        response= OkGo.<JSONObject>post  (getUrl()).headers(headers).upJson(json).execute();
                        break;
                    case "DELETE":
                        response= OkGo.<JSONObject>delete(getUrl()).headers(headers).upJson(json).execute();
                        break;
                    case "PUT":
                        response= OkGo.<JSONObject>put   (getUrl()).headers(headers).upJson(json).execute();
                        break;
                }
                return;
            }
            if("PARAM".equalsIgnoreCase(getRequest().getUpload())){
                ...
                sb.append("开始请求服务器!\t").append(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())).append("\n");
                sb.append("请求方式：").append(getRequest().getType()).append("\n");
                sb.append("请求地址：").append(getUrl()).append("\n");
                ...
                switch (getRequest().getType()){
                    case "GET":
                        response = OkGo.get   (getUrl()).headers(headers).params(params).tag(type).execute();
                        break;
                    case "POST":
                        response = OkGo.post  (getUrl()).headers(headers).params(params).tag(type).execute();
                        break;
                    case "DELETE":
                        response = OkGo.delete(getUrl()).headers(headers).params(params).tag(type).execute();
                        break;
                    case "PUT":
                        response = OkGo.put   (getUrl()).headers(headers).params(params).tag(type).execute();
                        break;
                }
    ...
    public void doRequest(List<Object> requestParams, ApiListener listener){
        ThreadPoolProvider.getFixedThreadPool().execute(()->{
            StringBuilder sb = new StringBuilder();
            doRequestInternal(requestParams, new ApiListener() {
                @Override
                public void onSuccess(int code, Object msg, StringBuilder sbLog) {
                    App.Post(()->listener.onSuccess(code, msg, sbLog));
                }

                @Override
                public void onFailure(int code, Object msg, StringBuilder sbLog) {
                    App.Post(()->listener.onFailure(code, msg, sbLog));
                }
            }, sb);
    ...
    protected void doResponse(Response response, ApiListener listener, StringBuilder sb){
        sb.append("\n\n开始解析返回数据：").append(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())).append("\n\n");
        boolean bIsResponsed = false;
        try {
            if(response == null) return;
            String strResponse = response.body().string();
            sb.append("response:").append(strResponse).append("\n");
            ESONObject result = new ESONObject(strResponse);
            String errMsg = result.getJSONValue("msg",result.getJSONValue("message",result.getJSONValue("errMsg",""))).trim();
            ...
            for(int i=0;i<lstAssets.size();++i){
                ...
                if(right.equalsIgnoreCase("null")){
                    if(!(isEq&&judge==null || !isEq && judge!=null)){
                        ...
                        bIsResponsed = true;
                        listener.onFailure(-1,errMsg.isEmpty()?"表达式未满足！":errMsg,sb);
                        return;
                    }
                }
            ...
            bIsResponsed = true;
            listener.onSuccess(0,entity,sb);
        } catch (Exception e) {
            ...
            if(!bIsResponsed){
                listener.onFailure(0,"数据请求失败！",sb);
            }
        }
    }
    //endregion

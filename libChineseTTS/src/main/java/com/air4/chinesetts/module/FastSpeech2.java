    ...
    public FastSpeech2(String modulePath) {
        try {
            ...
            for (int i = 0; i < input; i++) {
                ...
                Log.d(TAG, "input:" + i + " name:" + inputTensor.name() + " shape:" + Arrays.toString(inputTensor.shape()) +  " dtype:" + inputTensor.dataType());
            }

            int output = mModule.getOutputTensorCount();
            for (int i = 0; i < output; i++) {
                Tensor outputTensor = mModule.getOutputTensor(i);
                Log.d(TAG, "output:" + i +  " name:" + outputTensor.name() + " shape:" + Arrays.toString(outputTensor.shape()) + " dtype:" + outputTensor.dataType());
            }
